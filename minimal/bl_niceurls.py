...
def niceurlsspawn():
    ...
    return render_template('niceurls/niceurlsspawn.html', mc=mc, 
        page_title=page_title, page_title_for_id=page_title_for_id,user=current_user,show_footer=True)
...
def niceurl(slug=''):
    mc = set_menu("niceurls")
    page_title = "This is a title that will end up in the page url"
    return render_template('niceurls/pageniceurl.html', mc=mc, page_title=page_title,user=current_user,show_footer=True)
...
def niceurlid(slug='', id=0):
    mc = set_menu("niceurls")
    page_title_for_id = "This is a title that will end up in the page url"
    return render_template('niceurls/pageniceurlid.html', mc=mc, page_title_for_id=page_title_for_id,user=current_user,show_footer=True)
